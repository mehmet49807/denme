...
<div class="relationship-status-picker" role="radiogroup" aria-label="İlişki durumu">
    <label class="relationship-status-option {{ $selected === null || $selected === '' ? 'relationship-status-option--active' : '' }}">
    ...
    @foreach($options as $key => $meta)
        <label class="relationship-status-option {{ $selected === $key ? 'relationship-status-option--active' : '' }}">
            <input
                type="radio"
                name="{{ $name }}"
                value="{{ $key }}"
