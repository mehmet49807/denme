...
<div class="content-company-info" style="background:#f8f9fa;border:1px solid #e9ecef;border-radius:.5rem;padding:1.5rem;margin:1rem 0;">
    ...
    <p style="margin:0 0 .25rem;">
        @if(!empty($cTaxOffice))<strong>Vergi Dairesi:</strong> {{ $cTaxOffice }}@if(!empty($cTaxNumber)) · @endif@endif
        @if(!empty($cTaxNumber))<strong>VKN:</strong> {{ $cTaxNumber }}@endif
    </p>
    @endif
    @if(!empty($cMersis))
    ...
    <p style="margin:0 0 .25rem;">
        @if(!empty($cPhone))<strong>Tel:</strong> {{ $cPhone }}@if(!empty($cEmail)) · @endif@endif
        @if(!empty($cEmail))<strong>E-posta:</strong> <a href="mailto:{{ $cEmail }}">{{ $cEmail }}</a>@endif
    </p>
    @endif
