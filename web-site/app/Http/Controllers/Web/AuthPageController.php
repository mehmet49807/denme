...
class AuthPageController extends Controller
{
    ...
    public function resendEmailVerification(Request $request): RedirectResponse
    {
        ...
        $sentLink = false;
        try {
            $sentCode = $this->userMail->sendTwoFactorCode($user, $code);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('Email verification code send failed.', [
                'user_id' => $user->id,
                'exception' => $e::class,
            ]);
        }
        try {
            $sentLink = $this->userMail->sendEmailVerification($user, $this->emailVerificationUrl($user));
        } catch (\Throwable) {
            $sentLink = false;
        }

        $sent = $sentCode || $sentLink;

        return back()->with($sent ? 'status' : 'error', $sent
            ? 'E-posta adresinize 6 haneli doğrulama kodu gönderildi. Kodu aşağıya girerek doğrulayabilirsiniz.'
            : 'Doğrulama e-postası gönderilemedi. Lütfen daha sonra tekrar deneyin.');
    }

    /**
     * Profil doğrulama rozeti için e-posta kodu onayı.
     * Giriş engellemez; yalnızca email_verified_at işaretler.
     */
    public function verifyEmailCode(Request $request): RedirectResponse
    {
        ...
        if (! $user->hasValidTwoFactorCode()) {
            $user->clearTwoFactor();
            return back()->withErrors(['code' => 'Doğrulama kodunun süresi dolmuş. Yeni kod isteyin.']);
        }
        ...
        return back()->with('success', 'E-posta adresiniz doğrulandı. Doğrulanmış profil rozeti için admin onayını bekleyin.');
    }
    ...
    public function login(Request $request): RedirectResponse
    {
        ...
        if ($user && Hash::check($request->password, $user->password)) {
            ...
            // Kullanıcılar giriş sonrası tüm sayfaları görebilir (rozet için e-posta kodu ayrıca istenir).
            $user->clearTwoFactor();
